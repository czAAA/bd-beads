...
/**
 * Stamps a block onto the grid with its top-left corner at `at`, as one Pattern edit so it undoes in one step.
 * Deliberately not mirrored, whatever the Mirror axes say — the same precedent Fill sets (see CONTEXT.md's Mirror
 * entry): a live-mirrored stroke is a drawing aid, while stamping places a block exactly where it was aimed.
 * Returns the same Pattern instance, unchanged, when the stamp would leave every cell as it already is.
 */
